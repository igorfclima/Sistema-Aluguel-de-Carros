...
func (s *creditoService) ConcederCredito(req *dto.CreateCreditoRequest, usuarioID uint) (*model.ContratoCredito, error) {
    ...
    contrato, err := s.contratoRepo.FindByID(req.ContratoID)
    if err != nil || contrato == nil {
        return nil, errors.New("contrato não encontrado")
    }
    if contrato.Tipo != model.TipoComCredito {
        return nil, errors.New("este contrato não permite concessão de crédito")
    }
